...
class cluster(object):
    def __init__(self, nInput, initData = None, nInitCluster = 2, clusterGrowing = True, desiredLabels = [0,1]):
        ...
        # print('Initial Cluster: ', self.centroids)
        # pdb.set_trace()
    ...
    def growInput(self, constant = None):
        ...
        # print('no of input cluster: ', self.centroids.shape[1])
    ...
    def updateCentroid(self, newData, winClsIdx, addSupport = True):
        ...
        # self.centroids[winClsIdx] = (self.centroids[winClsIdx] - np.round(
        #                                         ((self.centroids[winClsIdx] - newData)/
        #                                          (self.support[winClsIdx] + 0.0001)),4))
        
        self.distance             = LA.norm(self.centroids - newData, axis=1)

    def calcSampleImportance(self, data):
        # data are in numpy array format
        nData = data.shape[0]
        sampleImportance = np.zeros((self.nCluster,nData),dtype=float).astype(np.float32)
        for i,iCentroid in enumerate(self.centroids):
            sampleImportance[i] = (np.exp(-LA.norm(iCentroid - data, axis=1)))
            # pdb.set_trace()
        # sampleImportance = sampleImportance/np.max(sampleImportance,0)               # equation 6 from paper STEM

        return sampleImportance
    ...
    def fit(self, trainData, epoch = 1, growClusterLocalIndicator = True):
        ...
        for iEpoch in range(0,epoch):
            ...
            for iData in range(0,nTrainData):
                ...
                if self.growClustInd and self.clusterGrowing and growClusterLocalIndicator:
                    importantSampleIdx.append(indices.item())
        ...
        self.importantSample = torch.unique(importantSampleIdx).tolist()

        # print('perm: ', shuffled_indices)
    ...
    def getCluster(self,testData):
        ...
        for i,iCentroid in enumerate(self.centroids):
            ...
            if iCentroid.shape[0] != testData.shape[1]:
                pdb.set_trace()
...
class smallAE():
    ...
    def nodeGrowing(self, nNewNode = 1, device = torch.device('cpu')):
        ...
        # print('no of nodes: ',nNewNodeCurr)
